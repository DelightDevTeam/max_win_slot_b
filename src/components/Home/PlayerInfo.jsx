  ...
  return (
    <div className='px-2 px-sm-5  d-flex  justify-content-between  '>
      {playerInfos.map((item)=>{
        return <NavLink className='d-flex flex-column align-items-center  align-items-center   text-decoration-none ' to={item.link}>
            <img className='playerInfoImg' src={item.img} />
            <span className='fw-bolder  text-light  playerInfoTitle'>{item.title}</span>
        </NavLink>
      })}
    </div>
  )
